...
def display_eyes(img_copy, left_eye, right_eye):
    ...
    after_overlay = image_overlay_with_transparence(img_copy, resized_left, location=(gl.left_eye_x - round(size_eye/2), gl.left_eye_y - round(size_eye/2)))
    after_overlay = image_overlay_with_transparence(after_overlay, resized_right, location=(gl.right_eye_x - round(size_eye/2), gl.right_eye_y - round(size_eye/2)))
    return after_overlay


def neutral_face(path):
    sample_img = cv2.imread(path)
    img_copy = sample_img.copy()
    h,w,_ = img_copy.shape

    left_eye = cv2.imread('images/right_eye.png')
    right_eye = cv2.imread('images/left_eye.png')
